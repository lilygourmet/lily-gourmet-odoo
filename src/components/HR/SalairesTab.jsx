...
export default function SalairesTab({ user }) {
  ...
  async function handleGenererExcel() {
    if (employesSociete.length === 0) {
      setError('Aucun employé déclaré pour cette société')
      return
    }
    ...
    try {
      ...
      const societeNom = societe?.nom || (societeFilter === 'LG' ? 'LG TRAITEUR' : 'L ET N Gourmet')
      ...
      // Construire la matrice (style identique au modèle)
      const matrice = []
      matrice.push([societeNom])
      matrice.push(['', '', '', '', '', 'A RABAT LE', new Date()])
      matrice.push([])
      matrice.push(['Objet : Ordre de virement'])
      matrice.push(['Merci de bien vouloir realiser les virements des salaires suivants :'])
      matrice.push([`A partir de notre compte numero ${compteSource || '...'}${banqueSource ? ' (' + banqueSource + ')' : ''}`])
      matrice.push([])
      matrice.push([])

      // Header tableau
      matrice.push(['Nom', '', 'Montant', 'RIB', '', 'Devise', 'Nb'])

      // Une ligne employé + une ligne banque/RIB
      for (const e of validEmp) {
        const m = parseFloat(montants[e.id])
        matrice.push([e.nom, '', m, '', '', 'DIRHAMS', 1])
        matrice.push([e.banque || '', '', e.rib || '', '', '', '', ''])
        matrice.push([])  // séparation
      }

      matrice.push([])
      matrice.push(['', '', total, '', '', 'TOTAL', validEmp.length])
      matrice.push([])
      matrice.push([`Soit ${total.toLocaleString('fr-FR')} dirhams`])
      matrice.push([])
      matrice.push(['Cordialement'])
      matrice.push(['La direction'])
      matrice.push([])
      matrice.push([])
      matrice.push(['', societeNom + (societe ? ` au capital de ${Number(societe.capital_dh).toLocaleString('fr-FR')} DH` : '')])
      if (societe) {
        matrice.push(['', societe.adresse])
        matrice.push(['', `RC: ${societe.rc} · Patente ${societe.patente} · IF: ${societe.if_num} · CNSS: ${societe.cnss}`])
        matrice.push(['', `ICE: ${societe.ice}`])
      }
      ...
      ws['!cols'] = [
        { wch: 35 },   // A - Nom / Banque
        { wch: 4 },
        { wch: 18 },   // C - Montant / RIB
        { wch: 4 },
        { wch: 4 },
        { wch: 12 },   // F - DIRHAMS
        { wch: 5 },    // G - Nb
      ]
      // Formatage des montants en nombres
