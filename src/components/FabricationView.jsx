...
const estBase = n => BASES.some(r => r.test(String(n || ''))) || basesEnPlus.includes(String(n || ''))
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 min-w-0">
        <div className={(petit ? 'text-[13.5px] font-semibold' : 'text-[15px] font-bold') + ' line-through opacity-70'}>
...
export default function FabricationView({ user, onLogout, onNavigate, activeView }) {
  ...
  const [stocks, stocksBases] = useMemo(() => {
    ...
    return [s, sb]
    // `basesPretes` n'est pas lu ici : il ne sert qu'à redemander le calcul
    // quand la liste des bases de Layla est arrivée (elle vit hors de React).
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const retirer = ordre => {
    if (faits[ordre.name]) return marquer(ordre.name, ordre.produit, ordre.qty)
    const cle = Object.keys(faits).find(c => ((faits[c] || {}).ordres || []).includes(ordre.name))
