        ...
        >
          <div className="sticky top-0 bg-cream/95 backdrop-blur-sm border-b border-line px-6 py-4 flex items-start justify-between gap-4 z-10">
            <div className="min-w-0 flex-1">
              ...
              <div className="font-fraunces italic text-[22px] font-medium text-ink leading-tight truncate">
                {order.client_name || '—'}
              </div>
...
function ItemBlock({
  ...
  const warningText = item.warnings?.text
  ...
  return (
    <div className={!isLast ? 'pb-6 border-b border-dashed border-line' : ''}>
      ...
      <div className="flex items-baseline justify-between gap-3 flex-wrap mb-2">
        ...
        >
          {item.quantity && item.quantity > 1 && (
            <span className="text-ink font-sans not-italic mr-2">×{item.quantity}</span>
          )}
          {shortName}
        </span>
      ...
      {(item.theme || item.age || item.message) && (
        <>
          <div className="border-t border-dashed border-line my-4"></div>
          <div className="space-y-2.5 mb-4">
            {item.theme && <InfoRow label="Thème" value={item.theme} />}
            {item.age && <InfoRow label="Âge" value={item.age} />}
            {item.message && <InfoRow label="Message" value={item.message} />}
          </div>
        </>
      )}
