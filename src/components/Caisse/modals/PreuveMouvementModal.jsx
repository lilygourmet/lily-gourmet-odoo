...
import { Paperclip, CheckCircle2, ExternalLink, AlertTriangle, FileText, Camera, Upload, X } from 'lucide-react'
    ...
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <h3 style={{ display: 'inline-flex', alignItems: 'center', gap: 6, margin: 0, fontSize: 16, fontWeight: 600, color: '#1a0f0a' }}>
            <Paperclip size={16} /> Preuve de paiement
          </h3>
        ...
        {hasProof && (
          <div style={{ marginBottom: 14, padding: 12, background: '#F0F8E8', border: '1px solid #C8E0AC', borderRadius: 8 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#27500A', fontWeight: 500, marginBottom: 8 }}><CheckCircle2 size={14} /> Preuve actuelle</div>
            <a href={mvt.proof_url} target="_blank" rel="noopener noreferrer" style={{
              display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#993556', textDecoration: 'underline'
            }}><ExternalLink size={14} /> Ouvrir la preuve dans un nouvel onglet</a>
        ...
        {status === 'no_proof_declared' && !hasProof && (
          <div style={{ marginBottom: 14, padding: 12, background: '#F0EEEA', border: '1px solid #D8D4CC', borderRadius: 8 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#4a3a30' }}>
              <AlertTriangle size={14} /> Tu as déclaré qu'il n'y avait pas de preuve pour ce mouvement.
            </div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <button type="button" onClick={pickFile} style={{
            ...
            {file ? <><FileText size={14} /> {file.name}</> : <><Camera size={14} /> Choisir une photo ou un PDF</>}
          </button>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button onClick={handleUpload} disabled={!file || uploading} style={{
            ...
            {uploading ? 'Upload en cours…' : <><Upload size={14} /> Uploader cette preuve</>}
          </button>
        </div>
        ...
        {status === 'pending' && !hasProof && (
          <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid #e5d8c3' }}>
            ...
            <button onClick={onDeclareNoProof} disabled={uploading} style={{
              ...
              <X size={14} /> Déclarer "Pas de preuve de paiement"
            </button>
          </div>
        )}
      </div>
