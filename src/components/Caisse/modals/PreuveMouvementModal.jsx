  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={e => e.stopPropagation()}>
        ...
        {/* Résumé mouvement */}
        <div style={{ padding: '10px 12px', background: '#F9F6F1', borderRadius: 6, fontSize: 12, color: '#3A3733', marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
            <span>{mvt.label}</span>
            <strong style={{ color: '#99201E' }}>− {Math.abs(mvt.amount)} dh</strong>
          </div>
          {mvt.category && <div style={{ fontSize: 11, color: '#6F6A60', marginTop: 4 }}>{mvt.category}</div>}
        </div>
        ...
        {hasProof && (
          <div style={{ marginBottom: 14, padding: 12, background: '#F0F8E8', border: '1px solid #C8E0AC', borderRadius: 8 }}>
            ...
            {mvt.proof_uploaded_at && (
              <div style={{ fontSize: 10, color: '#6F6A60', marginTop: 6 }}>
        ...
        {status === 'no_proof_declared' && !hasProof && (
          <div style={{ marginBottom: 14, padding: 12, background: '#F0EEEA', border: '1px solid #D8D4CC', borderRadius: 8 }}>
            ...
            <button onClick={onReset} style={{ marginTop: 8, fontSize: 11, padding: '4px 10px', background: 'white', border: '1px solid #D8D4CC', borderRadius: 6, cursor: 'pointer', color: '#6F6A60' }}>
              Annuler ce statut
            </button>
          </div>
        )}
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <button type="button" onClick={pickFile} style={{
            ...
            border: '2px dashed #E8E2D8', background: '#FAFAF8', cursor: 'pointer',
            color: '#6F6A60', fontSize: 13
          }}>
            {file ? `📄 ${file.name}` : '📷 Choisir une photo ou un PDF'}
          </button>

          {file && (
            <div style={{ fontSize: 11, color: '#6F6A60', marginTop: 6, textAlign: 'center' }}>
        ...
        {status === 'pending' && !hasProof && (
          <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid #E8E2D8' }}>
            ...
            <button onClick={onDeclareNoProof} disabled={uploading} style={{
              ...
              background: 'white', border: '1px solid #E8E2D8', cursor: 'pointer', color: '#6F6A60'
            }}>
