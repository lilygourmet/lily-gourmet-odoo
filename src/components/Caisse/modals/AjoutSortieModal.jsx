      ...
      {caisseOwner === 'meriem' && (
        <Field label="Y aura-t-il une facture à récupérer ?">
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button onClick={() => setHasFacture(false)} style={toggleBtn(hasFacture === false, '#F4F0EA', '#C4BFB6', '#3A3733')}>Non</button>
          </div>
        </Field>
      )}
...
function toggleBtn(active, bg, brd, txt) {
  return {
    ...
    border: `0.5px solid ${active ? brd : '#E8E2D8'}`,
...
function Field({ label, children }) {
  return (<>
    <div style={{ fontSize: 11, color: '#6F6A60', marginBottom: 4, marginTop: 10 }}>{label}</div>
    {children}
  </>)
}
...
export function ModalBox({ title, titleColor, titleIcon, onClose, children }) {
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: 'white', borderRadius: 12, padding: 24, maxWidth: 420, width: '100%', border: '0.5px solid #E8E2D8', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, color: titleColor || '#3A3733', fontSize: 15, fontWeight: 500 }}>
