  ...
  async function handleCreate(formData) {
    try {
      await createUser({
        ...
        perm_fabrication_prod: formData.permFabricationProd,
        perm_fabrication_annexe: formData.permFabricationAnnexe,
        ...
        perm_recettes: formData.permRecettes,
...
function UserCard({ user, isCurrentUser, onEdit, onResetPassword, onDelete, onHardDelete, onDuplicate, onConfigNavbar }) {
  ...
  if (user.perm_fabrication_annexe) perms.push('Fabrication Annexe 2')
...
function UserForm({ onSubmit, onCancel, initialData, isNew, teams = [], employes = [], duplicatedFromName = null, currentUser = null }) {
  ...
  const [formData, setFormData] = useState({
    ...
    permFabricationProd: initialData?.perm_fabrication_prod || false,
    permFabricationAnnexe: initialData?.perm_fabrication_annexe || false,
    ...
    permRecettes: initialData?.perm_recettes !== undefined ? initialData.perm_recettes : false,
      ...
      <div>
        ...
        <div className={`space-y-3 ${isAdmin ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <PermGroup emoji="🎂" title="Production & Calendrier" defaultOpen={true}>
            ...
            <PermCheckbox id="perm-fabrication-annexe" label="Fabrication Annexe 2" desc="Onglet « Fabrication Annexe 2 » : voir ce qui est sous son mini, fabriquer par tournées et déclarer ce qui est sorti." checked={isAdmin || formData.permFabricationAnnexe} onChange={v => update('permFabricationAnnexe', v)} />
            ...
            <PermCheckbox id="perm-recettes" label="Recettes" desc="Onglet « Recettes » : voir toutes les recettes et les essayer. Ne fabrique rien, n'écrit rien dans Odoo." checked={isAdmin || formData.permRecettes} onChange={v => update('permRecettes', v)} />
