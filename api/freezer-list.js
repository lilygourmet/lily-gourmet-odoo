...
async function parentsEncaisses(uid, jours, dry) {
  ...
  // Tous les gâteaux cake design : les « N étages », les Gateau Forme (cœur,
  // carré, bombé) et les grands formats 35 à 60 cm — Layla les a ajoutés le
  // 2026-08-29. La ganache est un ingrédient, pas un gâteau : elle reste dehors.
...
export default async function handler(req, res) {
  try {
    ...
    // ============================================================
    // Ce que DEVRAIENT être les mini/maxi de chaque préparation SM.
    //
    // On part de la CONSOMMATION RÉELLE — ce qui est sorti du stock vers la
    // production — et surtout PAS des ordres de fabrication : ceux-là sont déjà
    // pilotés par les règles en place, s'en servir pour les recalculer ne
    // ferait que confirmer les réglages existants.
    //
    //   mini = ce qui part en une journée où l'on s'en sert
    //          (total ÷ nombre de jours avec une sortie ; diviser par 90 jours
    //          de calendrier écraserait tout, la consommation est en rafales :
    //          médiane de 20 jours actifs sur 90 à la boutique, 9 à l'annexe)
    //   maxi = mini + une fournée habituelle, et au moins deux journées
    //
    // ⚠️ L'ordre de fabrication est souvent dans une unité MAISON
    // (« Tournée (3 kg) ») alors que la consommation est en grammes : sans
    // conversion, une fournée de 12 kg se lit « 4 ».
    // ============================================================
    if (req.query.mode === 'minmax') {
      const JOURS = 90
      const uid = await odooAuth()
      const dISO = (n) => {
      ...
      const estSM = (n) => /^\s*(\[[^\]]*\]\s*)?SM/i.test(n || '')
      const net = (n) => String(n || '').replace(/^\[[^\]]*\]\s*/, '').trim()
      const readGroup = (model, domain, fields, groupby, opts = {}) =>
        odooCall(uid, model, 'read_group', [domain, fields, groupby], { lazy: false, ...opts })
      const arrondi = (v, unite) => {
        if (!(v > 0)) return 0
        if (unite === 'u') return Math.max(1, Math.round(v))
        const pas = v < 1000 ? 50 : v < 10000 ? 100 : 500
        return Math.round(v / pas) * pas
      }
      const mediane = (a) => {
        if (!a || !a.length) return null
        const t = [...a].sort((x, y) => x - y); const i = Math.floor(t.length / 2)
        return t.length % 2 ? t[i] : Math.round((t[i - 1] + t[i]) / 2 * 100) / 100
      }

      const uoms = await odooSearchRead(uid, 'uom.uom', [], ['id', 'factor', 'category_id'],
        { limit: 500, context: { active_test: false } })
      const U = Object.fromEntries(uoms.map(u => [u.id, u]))
      const LIEUX = [
        { id: 52, nom: 'Boutique — Stock Prod' },
        { id: 62, nom: 'Annexe — Stock Prod annexe' },
      ]
      const sortie = []
      for (const L of LIEUX) {
        const g = await readGroup('stock.move', [
          ['state', '=', 'done'], ['date', '>=', dISO(JOURS)],
          ['location_id', 'child_of', L.id], ['location_dest_id.usage', '=', 'production'],
        ], ['product_qty'], ['product_id', 'date:day'], { limit: 20000 })

        const par = {}
        for (const x of g) {
          if (!x.product_id || !estSM(x.product_id[1])) continue
          const d = par[x.product_id[0]] ||= { nom: net(x.product_id[1]), jours: [] }
          d.jours.push(Math.abs(x.product_qty || 0))
        }
        const ids = Object.keys(par).map(Number)
        if (!ids.length) { sortie.push({ ...L, articles: [] }); continue }

        const [prods, quants, ops, mos] = await Promise.all([
          odooSearchRead(uid, 'product.product', [['id', 'in', ids]],
            ['id', 'name', 'active', 'uom_id'], { limit: 2000, context: { active_test: false } }),
          readGroup('stock.quant', [['location_id', 'child_of', L.id], ['product_id', 'in', ids]],
            ['quantity'], ['product_id'], { limit: 2000 }),
          odooSearchRead(uid, 'stock.warehouse.orderpoint', [['product_id', 'in', ids]],
            ['product_id', 'location_id', 'product_min_qty', 'product_max_qty'], { limit: 3000 }),
          odooSearchRead(uid, 'mrp.production', [
            ['product_id', 'in', ids], ['state', '=', 'done'],
            ['date_planned_start', '>=', dISO(180)],
          ], ['product_id', 'product_qty', 'product_uom_id'], { limit: 6000 }),
        ])
        const fiche = Object.fromEntries(prods.map(p => [p.id, p]))
        const stock = Object.fromEntries(quants.map(x => [x.product_id[0], x.quantity || 0]))
        const regle = {}
        for (const o of ops) {
          if (!regle[o.product_id[0]]) {
            regle[o.product_id[0]] = { min: o.product_min_qty, max: o.product_max_qty }
          }
        }
        const fournees = {}
        for (const m of mos) {
          const p = fiche[m.product_id[0]]; if (!p) continue
          const a = U[Array.isArray(m.product_uom_id) ? m.product_uom_id[0] : m.product_uom_id]
          const b = U[Array.isArray(p.uom_id) ? p.uom_id[0] : p.uom_id]
          if (!a || !b || !a.factor || !b.factor) continue
          if (a.category_id[0] !== b.category_id[0]) continue       // unités incomparables
          const q = (m.product_qty || 0) / a.factor * b.factor      // dans l'unité de l'article
          if (q > 0) (fournees[m.product_id[0]] ||= []).push(Math.round(q * 100) / 100)
        }

        const articles = []
        for (const id of ids) {
          const p = fiche[id]; if (!p) continue
          const js = par[id].jours
          const total = js.reduce((a, b) => a + b, 0)
          const parUtil = total / Math.max(js.length, 1)
          const f = mediane(fournees[id]) || 0
          const unite = ((Array.isArray(p.uom_id) ? p.uom_id[1] : '') || 'u').replace(/^units?$/i, 'u')
          const mini = arrondi(parUtil, unite)
          let maxi = arrondi(parUtil + (f >= parUtil ? f : parUtil), unite)
          if (maxi <= mini) maxi = arrondi(mini * 2, unite)
          const r = regle[id] || null
          const statut = !p.active ? 'archive'
            : js.length <= 3 ? 'demande'
              : !r ? 'creer'
                : (!r.min && !r.max) ? 'nulle' : 'revoir'
          articles.push({
            nom: par[id].nom, unite, statut,
            total: Math.round(total * 100) / 100,
            joursActifs: js.length,
            parUtil: Math.round(parUtil * 10) / 10,
            fournee: f || null,
            stock: Math.round((stock[id] || 0) * 100) / 100,
            regle: r, mini, maxi,
          })
        }
        articles.sort((a, b) => b.total - a.total)
        sortie.push({ ...L, articles })
      }
      // 90 jours d'historique ne bougent pas d'une minute à l'autre, et la
      // lecture coûte ~9 s : on la garde une heure.
      res.setHeader('Cache-Control', 'public, s-maxage=3600, stale-while-revalidate=86400')
      return res.status(200).json({ jours: JOURS, lieux: sortie })
    }
